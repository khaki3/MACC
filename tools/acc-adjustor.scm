...
(define (drop-brace-pair states)
  (and-let1 rest (and (string=? (car states) "{") (cdr states))
    (let loop ([braces 1] [rest rest] [ret '()])
      (if (= braces 0)
          (append (reverse (cdr ret)) rest)
          (let1 head (car rest)
            (loop (+ braces (count-braces head))
                  (cdr rest)
                  (cons head ret)))))))
    ...
    (if (null? states) (reverse! ret)
        ...
        (let ([line (car states)] [rest (cdr states)])
          (cond [(rxmatch #/^\#pragma acc (kernels|parallel|loop)/ line)
                 ...
                 (loop (cons line ret)
                       (if (string=? (car rest) "{") (drop-brace-pair rest) rest)
                       env
                       #t)]
                ...
                [(for? line)
                 (loop (cons (for-insert-declaration line env) ret) rest env #f)]
